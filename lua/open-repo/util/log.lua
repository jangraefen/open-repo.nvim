...
function log.notify(scope, level, verbose, str, ...)
  if not verbose and _G.OpenRepo.config ~= nil and not _G.OpenRepo.config.debug then
    return
  end
  ...
  vim.notify(
    string.format('[open-repo.nvim@%s] %s', scope, string.format(str, ...)),
    level,
    { title = 'open-repo.nvim' }
  )
end
...
function log.warn_deprecation(options)
  ...
  if uses_deprecated_option then
    ...
    log.notify('deprecated_options', vim.log.levels.WARN, true, 'use `:h OpenRepo.options` to read more.')
  end
end
